...
type SourceSpell = { description: String } & Record<string, unknown>;
...
async function merge_into_spells_data(input: { spells: Record<string, SourceSpell>; sources: {} }, dest_path: string) {
	...
	let dest_data = JSON.parse(await fs.readFile(dest_path, { encoding: "utf8" }));
	...
	for (const new_spell of Object.entries(input.spells)) {
		...
		const [desc_base, desc_higher_levels] = new_spell[1].description.split(AtHigherLevels, 2);
		dest_data.spells[new_spell[0]] = {
			...Object.fromEntries(KeysToKeepUnaltered.map((key) => [key[1], new_spell[1][key[0]]])),
			id: new_spell[0],
			description: { base: desc_base, higher_levels: desc_higher_levels },
		};
		console.log(dest_data, Object.keys(dest_data.spells).length);
	}
}
